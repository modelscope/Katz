...
namespace fast_kernel {
    namespace operators {
        ...
        __inline__ __device__ OutputType Cast(InputType x) {
        ...
        template<>
        struct DefaultComputeType<torch::Half> {
            using type = float;
        };
        ...
        struct DirectLoad {
            ...
            __device__ void load(DST* dst, int64_t row, int64_t col) const {
                ...
                #pragma unroll
                for (int i = 0; i < N; ++i) { dst[i] = Cast<SRC, DST>(pack.elem[i]); }
            }
            const SRC* src;
            int64_t row_size;
        };
        ...
        struct DirectStore {
            ...
            __device__ void store(const SRC* src, int64_t row, int64_t col) {
                ...
                const int64_t offset = (row * row_size + col) / N;
                #pragma unroll
                for (int i = 0; i < N; ++i) { pack.elem[i] = Cast<SRC, DST>(src[i]); }
                ...
                for (int row_id = 0; row_id < rows_per_access; ++row_id) {
                    ...
                    for (int pack_id = 0; pack_id < min_num_packs; ++pack_id) {
                        ...
                        for (int i = 0; i < pack_size; ++i) {
                            row_buf[pack_offset + i] = Cast<LoadType, ComputeType>(pack[i]);
            ...
            for (int64_t row = blockIdx.x; row < rows; row += gridDim.x) {
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    for (int i = 0; i < pack_size; ++i) {
                        ...
                        WelfordCombine(Cast<LoadType, ComputeType>(pack[i]), &thread_mean, &thread_m2, &thread_count);
                    }
                }
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    #pragma unroll
                    for (int i = 0; i < pack_size; ++i) {
                        pack[i] = (Cast<LoadType, ComputeType>(buf[i * num_packs + pack_id]) - row_mean) * row_inv_var;
                    }
        ...
        __global__ void __launch_bounds__(1024)
            ...
            for (int64_t row = blockIdx.x; row < rows; row += gridDim.x) {
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    #pragma unroll
                    for (int i = 0; i < pack_size; ++i) {
                        dst_pack[i] = (Cast<LoadType, ComputeType>(pack[i]) - row_mean) * row_inv_var;
                    }
                    ...
                    for (int pack_id = 0; pack_id < min_num_packs; ++pack_id) {
                        ...
                        for (int i = 0; i < pack_size; ++i) {
                            ...
                            row_normalized_buf[col_id] =
                                (Cast<LoadTypeX, ComputeType>(pack_x[i]) - mean_val) * inv_variance_buf[row_id];
                            row_dy_buf[col_id] = Cast<LoadTypeDy, ComputeType>(pack_dy[i]);
        ...
        __global__ void LayerNormGradBlockSMemImpl(LOAD_X load_x, LOAD_SCALED_DY load_scaled_dy,
            ...
            for (int64_t row = blockIdx.x; row < rows; row += gridDim.x) {
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    for (int i = 0; i < pack_size; ++i) {
                        ...
                        ComputeType normalized =
                            (Cast<LoadTypeX, ComputeType>(x_pack[i]) - mean_val) * inv_variance_val;
                        normalized_buf[buf_offset] = Cast<ComputeType, LoadTypeX>(normalized);
                        dy_buf[buf_offset] = dy_pack[i];
                        sum_stats1 += Cast<LoadTypeDy, ComputeType>(dy_pack[i]);
                        sum_stats2 += Cast<LoadTypeDy, ComputeType>(dy_pack[i]) * normalized;
                    }
                }
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    for (int i = 0; i < pack_size; ++i) {
                        const int buf_offset = i * num_packs + pack_id;
                        pack[i] = (cols * Cast<LoadTypeDy, ComputeType>(dy_buf[buf_offset]) - row_sum_stats1
                                   - Cast<LoadTypeX, ComputeType>(normalized_buf[buf_offset]) * row_sum_stats2)
                                  * inv_variance_over_cols;
                    }
                    store.template store<pack_size>(pack, row, pack_id * pack_size);
                }
        ...
        __global__ void LayerNormGradBlockUncachedImpl(LOAD_X load_x, LOAD_SCALED_DY load_scaled_dy,
            ...
            for (int64_t row = blockIdx.x; row < rows; row += gridDim.x) {
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    for (int i = 0; i < pack_size; ++i) {
                        sum_stats1 += Cast<LoadTypeDy, ComputeType>(dy_pack[i]);
                        sum_stats2 += Cast<LoadTypeDy, ComputeType>(dy_pack[i])
                                      * (Cast<LoadTypeX, ComputeType>(x_pack[i]) - mean_val) * inv_variance_val;
                    }
                }
                const ComputeType row_sum_stats1 = BlockAllReduce<SumOp, ComputeType, block_size>(sum_stats1);
                ...
                for (int pack_id = tid; pack_id < num_packs; pack_id += block_size) {
                    ...
                    for (int i = 0; i < pack_size; ++i) {
                        dx_pack[i] =
                            (cols * Cast<LoadTypeDy, ComputeType>(dy_pack[i]) - row_sum_stats1
                             - (Cast<LoadTypeX, ComputeType>(x_pack[i]) - mean_val) * inv_variance_val * row_sum_stats2)
                            * inv_variance_over_cols;
                    }
