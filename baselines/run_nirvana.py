...
#####################
NETWORK_OVERHEAD = 0.35
nirvana_config = {
    10: {"K": 10, "hit_ratio": 0.66},
    20: {"K": 20, "hit_ratio": 0.20},
}
#####################
...
parser.add_argument("--skipped-steps", type=int, default=10, choices=[10, 20], help="which nirvana config to use")
parser.add_argument("--enable-hit-ratio", action='store_true', default=False, help="whether to consider ratio")
parser.add_argument("--serve-mode", type=str, default="standard", choices=["standard"], help="which serve mode to use")
...
print("Nirvana config", nirvana_config[serve_args.skipped_steps])
...
##################### Inference #####################
cached_image_folder = "./images_sdxl_t2i_nirvana_cache"
...
for prompt_id, prompt in enumerate(prompts):
    ...
    cached_image = load_image(os.path.join(cached_image_folder, f"image_{prompt_id}.png")).resize((1024, 1024))
    ...
    images = pipe(
        prompt, negative_prompt=negative_prompt, image=cached_image, controlnet_conditioning_scale=controlnet_conditioning_scale,
        control_image=ref_image,
        generater=sd_generator,
        strength = 1.0 - nirvana_config[serve_args.skipped_steps]["K"]/50.0,
        serve_args=serve_args,
    ).images
